...
export default function GymPage() {
  ...
  useEffect(() => {
    ...
    setUser(parsedUser);
    loadTodayWorkouts(parsedUser.id);
  }, [router]);
  ...
  const loadTodayWorkouts = async (userId: string) => {
    try {
      ...
      console.log('No gym logs for today');
    } finally {
      setLoading(false);
    }
  };
  ...
  const resetForm = () => {
    ...
    setCaloriesBurned(0);
    setNotes('');
    // Don't reset checklist items - they persist for the day
  };
  ...
  const updateChecklist = async (field: 'warmup_done' | 'cooldown_done' | 'meditation_done', value: boolean) => {
    ...
    // Update the first workout's checklist (or all of them)
    try {
  ...
  useEffect(() => {
    // Load checklist state from first workout
    if (gymLogs.length > 0) {
      const firstLog = gymLogs[0];
  ...
  const exerciseNames = [...new Set(gymLogs.map(log => log.exercise_name))];
  ...
  return (
    <div className="container-pixel">
      ...
      {gymLogs.length === 0 && !showAddForm && (
        <Card>
          <div className="text-center py-8">
            <Dumbbell size={48} className="mx-auto mb-4 text-darkgray/30" />
            <p className="font-mono text-lg text-darkgray/70">No workouts logged today</p>
